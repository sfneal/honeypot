...
# Build the image
echo "Building image: stephenneal/honeypot:${TAG}"
docker build -t stephenneal/honeypot:"${TAG}" \
...
while true; do
    if [[ $(docker inspect -f '{{.State.Running}}' honeypot) != true ]]; then
        break
    else
        echo "'honeypot' container is still running... waiting 3 secs then checking again..."
        sleep 3
    fi
done

# Confirm it exited with code 0
if [[ $(docker inspect -f '{{.State.ExitCode}}' honeypot) == 0 ]]; then
    echo "Success: Tests Passed! - stephenneal/honeypot:${TAG}"
else
    echo "Error: Tests Failed! - stephenneal/honeypot:${TAG}"
    exit 1
fi

# Confirm the image exists
docker image inspect stephenneal/honeypot:"${TAG}" > /dev/null 2>&1
